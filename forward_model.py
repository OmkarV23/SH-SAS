...
def render_transmittance_from_density(radii, scatterers_to, occlusion_scale, factor2=False):

  ''' Scatterers_to is the density'''
  ...
  alpha = 1.0 - torch.exp(-(scatterers_to.abs() * radii_to[:, None] * occlusion_scale))
  trans_prob = cumprod_exclusive((1 - alpha + 1e-10).permute(1, 0)).permute(1, 0)

  return [trans_prob, alpha]

def render_transmittance_from_opacity(radii, scatterers_to, occlusion_scale, factor2=False):
  ...
  trans_prob = cumprod_exclusive((1 - scatterers_to + 1e-10).permute(1, 0)).permute(1, 0)

  return [trans_prob]

def render_transmittance_from_alpha(radii, scatterers_to, occlusion_scale, factor2=False):
  radii_to = radii[1:] - radii[:-1]
  radii_to = torch.cat((radii_to, torch.tensor([torch.mean(radii_to)]).to(radii.device)), dim=-1)
  ...
  # alpha = 1.0 - torch.exp(-(sigma * radii_to[:, None] * occlusion_scale))
  trans_prob = cumprod_exclusive((1 - scatterers_to + 1e-10).permute(1, 0)).permute(1, 0)
