...
if __name__ == '__main__':
    ...
    parser.add_argument('--object', required=False, default=None,
                        choices=['cube', 'new_sim'],
                        help="Object to simulate, either cube or new_sim")
    parser.add_argument('--wfm_dir', required=False, default=None,
                        help="Directory containing waveforms to use for reconstruction")
    ...
    parser.add_argument('--signal_snr', required=True,
                        default=20)
    parser.add_argument('--wfm_part_1', required=False, default=None,
                        help="Waveform part 1 to use for reconstruction")
    ...
    assert args.wfm_bw in ['5', '10', '20', '40', 'ultra']
    ...
    if args.wfm_bw == '5':
        ...
        # use center freq as 25000. f_start = 5000, f_stop = 45000
        wfm = gen_real_lfm(system_data[c.SYS_PARAMS][c.FS],
                           45000,
                           5000,
                           system_data[c.WFM_PARAMS][c.T_DUR],
    ...
    if args.object == 'cube':
        ...
        wfms = wfms / (2048 ** 2)
    elif args.object == 'new_sim':
        if args.wfm_dir is None:
            raise IOError("No waveform directory provided for new simulation")
        print("Loading waveforms from directory", args.wfm_dir)
        wfm_path = glob.glob(os.path.join(args.wfm_dir, '*.npy'))
        transient_files = sorted(wfm_path, key=lambda x: int(os.path.basename(x).split('_')[-1].split('.')[0])) #transeints_0.npy
        if len(transient_files) == 0:
            raise IOError("No waveforms found in directory {}".format(args.wfm_dir))
        wfms = []
        for p in tqdm(transient_files, desc="Loading waveforms"):
            wfms.append(np.load(p))
        wfms = np.array(wfms)
    else:
