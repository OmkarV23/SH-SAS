...
class MIDataSchema:
    def __init__(self):
        self._data = {
            c.GEOMETRY: None,  # points to a class Geometry instantiation (see classes above)
            c.IMPULSE_RESPONSE: None,  # Impulse response of the system, size [num_samples, num_freq]
            c.NUM_FREQ: None,  # Number of frequency points in the impulse response
            c.WAVE_NUMBER: None,  # Wave number (k = 2 * pi / wavelength) in 1/m [num_freq]
            c.LIGHT_SPEED: None,  # Speed of light in m/s
            c.SYS_PARAMS: None,  # points to a class SysParams instantiation (see classes above)
            c.TX_BW: None,  # beamwidth of the transmitters in degrees
            c.RX_BW: None,  # beamwidth of the receivers in degrees
            c.TX_COORDS: None,  # position of transmitters in meters, size num_sensor_pos x 3 
            c.RX_COORDS: None,  # position of receivers in meters, size num_sensor_pos x 3
            #Store two way distances. Need for importance sampling
            c.MIN_DIST: None,  # Minimum distance from the transducer to the scene
            c.MAX_DIST: None,  # Maximum distance from the transducer to the scene
        }
