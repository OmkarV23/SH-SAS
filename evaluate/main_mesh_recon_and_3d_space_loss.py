...
# Precision / Recall / F1 on voxel occupancy
def calculate_precision_recall_f1(voxels_gt, points, prefix=""):
    voxels_pred = point_cloud_to_voxel(points, unit=0.005)

    tp = np.logical_and(voxels_pred, voxels_gt).sum()
    fp = np.logical_and(voxels_pred, np.logical_not(voxels_gt)).sum()
    fn = np.logical_and(np.logical_not(voxels_pred), voxels_gt).sum()
    ...
    return {
        prefix + "precision": float(precision),
        prefix + "recall": float(recall),
        prefix + "f1": float(f1),
    }
...
def calculate_3d_loss(args):
    ...
    prf_loss = calculate_precision_recall_f1(gt_voxels, inferred_points)
    prf_loss_mesh = calculate_precision_recall_f1(gt_voxels, mesh_points, prefix="mesh_")

    loss_dict[args.expname] = {**cham_loss, **iou_loss, **cham_loss_mesh, **iou_loss_mesh, **prf_loss, **prf_loss_mesh}
