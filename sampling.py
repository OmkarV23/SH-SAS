...
class SceneSampler:
    ...
    def ellipsoidal_sampling(self, radii, tx_pos, rx_pos, num_rays, scene_bounds,
        ...
        if create_return_vec:
            ...
            # model_out = scene_model(vec_to_world.reshape(-1, 3)*scene_scale_factor, 
            #                         view_dir = -dir_to_world.unsqueeze(0).expand(num_rad, num_ray, -1) if use_view_net else None,
            #                         compute_normals=compute_normals)

            model_out = scene_model(positions=vec_to_world.reshape(-1,3)*scene_scale_factor,
                                    rx_pos=rx_pos.to(device).reshape(-1, 3).float())

            scatterers_to = model_out['scatterers_to']
            normals = model_out['normals']
            density = model_out['sigma']
            sh_coeffs = model_out['sh_coeffs']

            scatterers_to = scatterers_to.reshape(num_rad, num_ray)
            density = density.reshape(num_rad, num_ray)

            out = transmission_model(radii=radii,
                                    scatterers_to=density,
                                    occlusion_scale=occlusion_scale)
            alpha = None
            transmission_probs = out[0]
            if len(out) > 1:
                alpha = out[1]
            ...
            # scatterers_back, density = scene_model.model_out(vec_back_world.reshape(-1, 3)*scene_scale_factor,
            #                                         view_dir=--dir_back.unsqueeze(0).expand(num_rad, num_rays, -1) if use_view_net else None,)

            model_out_back = scene_model(positions=vec_back_world.reshape(-1, 3)*scene_scale_factor,
                                            rx_pos=rx_pos.to(device).reshape(-1, 3).float(), 
                                            compute_normals=False)
            
            scatterers_back = model_out_back['scatterers_to']
            # normals_back = model_out_back['normals']
            density_back = model_out_back['sigma']
            ...
            density_back[ignore_return_scatterers_index] = \
                density_back[ignore_return_scatterers_index] * 0.
            
            density_back = density_back.reshape(num_rad, num_ray)

            back_trans_probs = transmission_model(radii=radii,
                                scatterers_to=density_back,
                                occlusion_scale=occlusion_scale)[0]
