...
def config_parser():
    ...
    parser.add_argument('--model_type', required=False, type=str, choices=['nvrcsas', 'sh_mlp'],
                        help='Type of model to use')
    parser.add_argument('--rand_batch', required=False, type=int, default=0,
                        help='Random batch size')
    parser.add_argument('--learning_rate', required=False, type=float, default=1e-4,
                        help='Learning rate for the INR')
    parser.add_argument('--lr_scheduler', required=False, action='store_true',
    ...
    parser.add_argument('--use_aabb', required=False, action='store_true',
                         help="Whether to use an axis aligned bounding box for the scene")
    ...
    parser.add_argument('--resample_measurements', required=False, action='store_true', default=False,
                        help="Whether to resample cylindrical measurements")
    parser.add_argument('--skip_every_n', required=False, default=4, type=int,
                        help="Skip every --skip_every_n measurements to for sparse view resampling. ")
    ...
    parser.add_argument('--lambertian_ratio', '--lambertion_ratio',
                        required=False, type=float, default=0., help="0 is use full lambertian, "
                                                                                         "1 is no lambertian")
    parser.add_argument('--sh_levels', required=False, type=int, default=3,
                         help="Number of spherical harmonics levels to use for the scene")
    parser.add_argument('--sh_loss', required=False, default=None, type=float,
                        help="Weight of spherical harmonics loss")
    parser.add_argument('--SH_grad_scale', required=False, action='store_true', default=False,
                        help="Whether to scale gradients for spherical harmonics")
    parser.add_argument('--progressive_SH', required=False, action='store_true', default=False,
                        help="Whether to progressively increase SH levels during training")
    parser.add_argument('--post_convolve', required=False, action='store_true', default=False,
                         help="Whether to convolve the scene with the post convolution kernel")
    parser.add_argument('--load_wfm', required=False, default=None, type=str, 
                         help="Path to load waveform from. Required if --post_convolve is set to True")
    ...
    parser.add_argument('--use_view_net', required=False, action='store_true', help="Whether to use a view dependent network")
