...
def wfm_kernel(wfm, device='cuda'):
    m = torch.nn.Conv1d(1, 1, wfm.shape[0], padding=(wfm.shape[0] - 1) // 2, padding_mode="zeros", device=device)
    m.weight.requires_grad = False
    m.bias.requires_grad = False
    m.bias *= 0
    m.weight = torch.nn.Parameter(wfm[None, None, ...].to(dtype=wfm.dtype))
    return m