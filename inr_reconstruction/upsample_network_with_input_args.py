...
if __name__ == '__main__':
    ...
    if args.view_model:
        if args.max_voxels is not None:
            ...
            comp_albedo = np.zeros((all_scene_coords.shape[0]), dtype=float)
            for chunk in tqdm(chunks, desc="Querying network for full scene..."):
                scene_chunk = all_scene_coords[chunk, :]
                with torch.no_grad():
                    model_out = scene_model.query_density(scene_chunk.cuda().view(-1,3))['sigma']

                comp_albedo[chunk] = model_out.detach().cpu().numpy()
                del scene_chunk
        else:
            model_out = scene_model.query_density(all_scene_coords.cuda().view(-1,3))['sigma']
            comp_albedo = model_out.detach().cpu().numpy()
    ...
    mask = mag > args.threshold
    mag = (mag - mag.min()) / (mag.max() - mag.min())
    condition = mag > args.threshold

    output_dir = os.path.join(experiment_dir_agave, 'nbp_output', exp, args.output_dir_name)
